...
int CurrentSense::alignBLDCDriver(float voltage, BLDCDriver* bldc_driver, bool modulation_centered) {
  ...
  if (max_c_ratio >= 1.5f) {
    ...
    //phase A is not measured so put the _NC to the phase A
  ...
  if (max_c_ratio >= 1.5f) {
    ...
    //phase B is not measured so put the _NC to the phase B
  ...
  if (_isset(pinC)) {
    if (_sign(c_b.c) > 0) {
      // the expected current is -I/2 (if the phase A and B are aligned and C has correct polarity)
