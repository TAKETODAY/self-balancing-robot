...
tccConfiguration tccConfs[2] = {
  getTCCChannelNr(pinA, getPeripheralOfPermutation(compatibility, 0)),
...
tccConfiguration tccConfs[4] = {
  getTCCChannelNr(pin1A, getPeripheralOfPermutation(compatibility, 0)),
...
return new SAMDHardwareDriverParams {
  .
  tccPinConfigurations = {
    getTccPinConfiguration(pinA_h), getTccPinConfiguration(pinA_l), getTccPinConfiguration(pinB_h), getTccPinConfiguration(pinB_l), getTccPinConfiguration(pinC_h), getTccPinConfiguration(pinC_l)
  },
  .
  pwm_frequency = (uint32_t) pwm_frequency,
  .
...
void printAllPinInfos() {
  ...
  for (uint8_t pin = 0; pin < PINS_COUNT; pin++) {
    ...
    switch (pinDesc.ulPort) {
      case NOT_A_PORT: SimpleFOCDebug::print("    ");
        break;
      case PORTA: SimpleFOCDebug::print("  PA");
        break;
      case PORTB: SimpleFOCDebug::print("  PB");
        break;
      case PORTC: SimpleFOCDebug::print("  PC");
        break;
#if defined(_SAMD51_)||defined(_SAME51_)
