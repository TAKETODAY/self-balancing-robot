...
public class JoystickView extends View implements Runnable {
   ...
   */
  private @Nullable OnMultipleLongPressListener mOnMultipleLongPressListener;

  private final Handler multipleLongPressHandler = new Handler();

  private final Runnable runnableMultipleLongPress;
  ...
  public JoystickView(Context context, AttributeSet attrs) {
    ...
    try (TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs,
            R.styleable.JoystickView, 0, 0)) {

      buttonColor = attributes.getColor(R.styleable.JoystickView_buttonColor, DEFAULT_COLOR_BUTTON);
      borderColor = attributes.getColor(R.styleable.JoystickView_borderColor, DEFAULT_COLOR_BORDER);
      mBorderAlpha = attributes.getInt(R.styleable.JoystickView_borderAlpha, DEFAULT_ALPHA_BORDER);
      backgroundColor = attributes.getColor(R.styleable.JoystickView_backgroundColor, DEFAULT_BACKGROUND_COLOR);
      borderWidth = attributes.getDimensionPixelSize(R.styleable.JoystickView_borderWidth, DEFAULT_WIDTH_BORDER);
      mFixedCenter = attributes.getBoolean(R.styleable.JoystickView_fixedCenter, DEFAULT_FIXED_CENTER);
      mAutoReCenterButton = attributes.getBoolean(R.styleable.JoystickView_autoReCenterButton, DEFAULT_AUTO_RECENTER_BUTTON);
      mButtonStickToBorder = attributes.getBoolean(R.styleable.JoystickView_buttonStickToBorder, DEFAULT_BUTTON_STICK_TO_BORDER);
      buttonDrawable = attributes.getDrawable(R.styleable.JoystickView_buttonImage);
      mEnabled = attributes.getBoolean(R.styleable.JoystickView_enabled, true);
      mButtonSizeRatio = attributes.getFraction(R.styleable.JoystickView_buttonSizeRatio, 1, 1, 0.25f);
      mBackgroundSizeRatio = attributes.getFraction(R.styleable.JoystickView_backgroundSizeRatio, 1, 1, 0.75f);
      mButtonDirection = attributes.getInteger(R.styleable.JoystickView_buttonDirection, BUTTON_DIRECTION_BOTH);
    }
    ...
    switch (event.getActionMasked()) {
      ...
      case MotionEvent.ACTION_POINTER_DOWN: {
        ...
        if (event.getPointerCount() == 2) {
          multipleLongPressHandler.postDelayed(runnableMultipleLongPress, ViewConfiguration.getLongPressTimeout() * 2L);
      ...
      case MotionEvent.ACTION_MOVE:
        mMoveTolerance--;
        if (mMoveTolerance == 0) {
          multipleLongPressHandler.removeCallbacks(runnableMultipleLongPress);
        }
        break;
