...
public abstract class AbstractFuture<V> extends Future<V> {
  ...
  public V get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
    ...
    if (s <= COMPLETING && (s = awaitDone(true, unit.toNanos(timeout))) <= COMPLETING) {
      throw new TimeoutException(String.format("Timeout, after %s seconds", unit.toSeconds(timeout)));
    }
    return report(s);
  }
  ...
  public boolean trySuccess(@Nullable V v) {
    if (STATE.compareAndSet(this, NEW, COMPLETING)) {
      result = v;
      STATE.set(this, NORMAL); // final state
      finishCompletion();
      return true;
    }
    return false;
  ...
  public boolean tryFailure(Throwable t) {
    if (STATE.compareAndSet(this, NEW, COMPLETING)) {
      ...
      result = t;
      STATE.set(this, EXCEPTIONAL); // final state
      finishCompletion();
      return true;
    }
    return false;
  ...
  public String toString() {
    final String status = switch (state) {
      ...
      case EXCEPTIONAL -> String.format("[Completed exceptionally: %s]", result);
  ...
  private static final AtomicIntegerFieldUpdater<AbstractFuture> STATE = AtomicIntegerFieldUpdater.newUpdater(AbstractFuture.class, "state");
  private static final AtomicReferenceFieldUpdater<AbstractFuture, Waiter> WAITERS = AtomicReferenceFieldUpdater.newUpdater(
