...
public abstract class Future<V> implements java.util.concurrent.Future<@Nullable V> {
   ...
   * {@link Stream#parallel()} and {@link CompletableFuture}}
   * </li>
   * </ul>
   *
    ...
    if (!isDone()) {
      try {
        if (!await(timeout.toNanos(), NANOSECONDS)) {
          ...
          throw new TimeoutException(String.format("Timeout on blocking read for %s ms", timeout.toMillis()));
        }
      }
  ...
  public static FutureCombiner combine(Stream /*<Future<?>>*/ futures) {
    Assert.notNull(futures, "Futures is required");
    return new FutureCombiner(true, (Collection<Future<?>>) futures.collect(Collectors.toList()));
  }
